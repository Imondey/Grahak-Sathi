...
// ── Capture-match decision thresholds (default; overridable per store) ───────────
// The 0–1 confidence that the item photographed at checkout matches the scanned
// SKU's reference image drives a three-way decision. These are the GLOBAL
// defaults; each store can override them (retailers table + Redis) so a store
// can tune fraud sensitivity without a code change:
//   confidence >  APPROVE            -> auto-approve (post-approval actions run)
//   BLOCK < confidence <= APPROVE    -> manager review (held for a human)
//   confidence <= BLOCK              -> auto-block (fraud-alert flow)
const CAPTURE_AUTO_APPROVE_THRESHOLD = num(process.env.CAPTURE_AUTO_APPROVE_THRESHOLD, 0.90);
const CAPTURE_AUTO_BLOCK_THRESHOLD   = num(process.env.CAPTURE_AUTO_BLOCK_THRESHOLD,   0.60);
...
module.exports = {
    ...
    CAPTURE_AUTO_APPROVE_THRESHOLD, CAPTURE_AUTO_BLOCK_THRESHOLD,
