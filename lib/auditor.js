    ...
    async function handleMessage(p) {
        ...
        if (!skipInjection) {
            ...
            if (ml && ml.available) {
                ...
                const dec0  = router.route('injection_classify', { budget: shopId != null ? await budget.routingSnapshot(sessionId, shopId) : snap0 });
                if (!dec0.denied) {
                    const charge = await budget.charge(sessionId, { tier: dec0.tier, taskType: 'injection_classify', cost: dec0.estCost, shopId });
        ...
        const intentDecision = router.route('intent_parse', { budget: shopId != null ? await budget.routingSnapshot(sessionId, shopId) : snap1 });
        ...
        if (intentDecision.tier === 'medium' && !intentDecision.denied) {
            ...
            const charge = await budget.charge(sessionId, { tier: intentDecision.tier, taskType: 'intent_parse', cost: intentDecision.estCost, shopId });
        ...
        if (parsed.intent === 'faq' || parsed.intent === 'other') {
            ...
            const decF  = router.route('faq', { budget: shopId != null ? await budget.routingSnapshot(sessionId, shopId) : snapF });
            routing.push(decF);
            const charge = await budget.charge(sessionId, { tier: decF.tier, taskType: 'faq', cost: decF.estCost, shopId });
        ...
        const visDec = router.route('auditor_vision', { budget: shopId != null ? await budget.routingSnapshot(sessionId, shopId) : snap2 });
        routing.push(visDec);

        if (visDec.tier !== 'high' && customerImage) {
            const charge = await budget.charge(sessionId, { tier: visDec.tier, taskType: 'auditor_vision', cost: visDec.estCost, shopId });
