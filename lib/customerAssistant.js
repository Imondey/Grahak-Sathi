...
function createCustomerAssistant({
    ...
    async function chargeStep(sessionId, shopId, taskType, snapshot, latencyMs = null) {
        // Route against the combined session + monthly-store budget when a shop
        // is known, so conversational AI degrades if EITHER budget is low.
        const routeBudget = shopId != null ? await budget.routingSnapshot(sessionId, shopId) : snapshot;
        const decision = router.route(taskType, { budget: routeBudget });
        if (!decision.denied) {
            const c = await budget.charge(sessionId, { tier: decision.tier, taskType, cost: decision.estCost, shopId });
    ...
    async function handle(p) {
        ...
        if (ml && ml.available) {
            ...
            const dec0  = router.route('injection_classify', { budget: shopId != null ? await budget.routingSnapshot(sessionId, shopId) : snap0 });   // light/local
            if (!dec0.denied) {
                const c = await budget.charge(sessionId, { tier: dec0.tier, taskType: 'injection_classify', cost: dec0.estCost, shopId });
        ...
        const aiDec  = router.route('general_chat', { budget: shopId != null ? await budget.routingSnapshot(sessionId, shopId) : snapAI });
        routing.push(aiDec);
        ...
        // A failed / empty completion is NOT billable — don't charge the budget
        // and don't show a cost for it. Zero the routing step so the transparency
        // panel reflects reality, then hand off to a human.
        if (!answer) {
            aiDec.estCost = 0;
            aiDec.reason  = 'Medium-tier LLM returned no answer — not charged; handed off to support.';
            console.warn(`⚠  general_chat LLM returned empty (session ${sessionId}) — skipping charge.`);
            return finalize({
        ...
        // The gateway returned a completion → bill it (even if we then reject the
        // content as a prompt-leak below: the tokens were already spent).
        const charge = await budget.charge(sessionId, { tier: aiDec.tier, taskType: 'general_chat', cost: aiDec.estCost, shopId });
        ...
        if (injection.looksLikePromptLeak && injection.looksLikePromptLeak(answer)) {
